...
**Date:** 2026-09-05 · **Status:** Current (v5 — learning-release journal/provider contracts)
...
The learning blueprint and ADR-023 govern implemented scope. Detailed production designs below retain deferred options; the LRN-06/07 decisions in §17.6 supersede conflicting journal and provider examples.
...
The LRN-06/07 learning implementation freezes the model vocabulary and error contract in
`packages/protocol`, and the journal implementation API in `packages/storage`. Its pure fold
lives in `packages/session`. Providers and session each import protocol only. The CLI composes
adapters; the kernel must not import concrete providers or storage.

`ModelProvider.capabilities()` and the compatibility package are deferred. The current journal
is typed-entry JSONL, not entity patches, snapshots or SQLite. Tool/policy interfaces below are
future implementation contracts; Directors remain deferred under ADR-023.

Every append fsyncs before acknowledgement; full verification precedes writes and sequence
filtering. A permanent sibling inode carries an OS-released BSD advisory lock through the
storage-only `fs-native-extensions@1.5.0` addon. The Rust executor still arrives in M4.
Repair first preserves a damaged original, then atomically publishes the verified prefix plus
its repair record. It never auto-repairs terminated corruption or incompatible schemas.

Assistant message v2 and `ProviderError.partial` share a lossless raw-tool response snapshot
with an explicit complete/interrupted outcome; v1 remains supported. Late usage-only chunks
are consumed before terminal success. See the [implementation evidence](lrn-06-07-implementation-evidence.md).
...
interface JournalReader {
  read(sessionId: string, fromSeq?: number): AsyncIterable<ReadableRecord>;
  readAll(sessionId: string, fromSeq?: number): Promise<JournalRead>; // includes tail diagnostics
}
