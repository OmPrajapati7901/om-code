...
| **LRN-18** | Multi-tool turn: tool calls execute, results return to the model, the loop continues until it answers. Parallel calls run concurrently | Core | M | 10, 17 | LR-FR-019 | **`om run -p "where is auth handled?"` gives a correct answer on a real repo of yours** | half completed | Code green, full gate green (`pnpm run check` exit 0, 28 kernel + 56 cli + 76 root tests, coverage printed). Live run 2026-09-07 (Groq `qwen/qwen3.8-27b`) proved the loop from the journal: 2 parallel calls (grep+glob) executed with per-call `tool_call.seq < tool_result.seq`, both results fed back, second inference issued a follow-up read; failed calls (glob dialect refusal, read invalid-input) continued the turn per AC-18.4. Left: the final answer was blocked by Groq OTPM 429 quota (journaled provider/http, retryable), not by code — retry the one-shot gate on `openai/gpt-oss-20b` (the LRN-11 precedent) before flipping to completed. |
| **LRN-19** | **Central bounding.** The runtime — not any tool — truncates results, notes the truncation and spills the rest to a file. Turn count, wall-clock and byte caps live in this one module | Core | M | 18 | LR-FR-023, LR-NFR-003 | A 50 MB command output cannot blow the context. **Zero truncation logic inside any tool** | half completed | Code green, full gate green (`pnpm run check` exit 0; context 11/11 incl. 50–52 MB fuzz with zero violations, kernel 33/33, storage 63+1 skipped, cli 60/60, root 78/78; coverage printed). `packages/context` owns limits/bounder/budget/cost; blob spill at `<OM_HOME>/blobs/sha256/<ab>/<hash>` (0600/0700, idempotent, hash-verified on read, rendered by `om show`); `--max-turns` re-scoped to agent iterations (exit 3 + journaled reason), `--max-cost` startup-rejects on missing pricing (exit 1) and enforces/aborts mid-run (exit 3 / exit 1 unknown-usage); AC-19.1 grep lint green with the two benign forms scrubbed by name. Left: paid manual e2e (`om run` blob spill listing, `--max-turns 2` exit 3, `--max-cost` paths) — needs endpoint-credit approval. |
