...
| **DoD-3** | `pnpm biome check`, `tsc --noEmit`, `dependency-cruiser` all clean | CI green on the branch. CI arrives in LRN-15; before it lands, this gate means the checks that exist, run locally (`pnpm run lint` = `biome check .` plus `lint:deps`) |
