...
| **DoD-3** | `pnpm biome check`, `tsc --noEmit`, `dependency-cruiser` all clean | CI green on the branch (`.github/workflows/ci.yml`, single macOS arm64 gate since LRN-15) |
...
| **LRN-11** | `packages/cli`: `om run` **interactive (a real multi-turn REPL) and `-p` one-shot**, streamed output, `om sessions`, `om show <id>`, and the exit-code contract 0/1/2/3/4 | Core | M | 10 | LR-FR-005 | **`om run` holds a multi-turn conversation against your real endpoint and leaves a journal you can read.** Codes 0/1/3 are tested here; 2 and 4 cannot exist until policy (LRN-21a–d, LRN-22) and are tested there | completed | Async injected composition root, REPL/one-shot streaming, journal-derived multi-turn context, project session listing/show, NDJSON journal tee, and exit codes 0/1/3 landed. `pnpm run check` green: 325 passed/1 skipped, including 48 CLI tests and the no-network guard. Live Groq verification on macOS arm64 used `openai/gpt-oss-20b`: multi-turn responses streamed, a deterministic in-flight SIGINT printed `om: turn interrupted; session kept`, the same process answered `recovered after SIGINT`, EOF exited 0, `om sessions` listed newest-first idle sessions, `om show` rendered `Assistant [interrupted]` plus the recovery without inference, and the one-shot NDJSON-to-`jq -e` pipeline exited 0. The configured Qwen model was not used for the successful run because its current Groq on-demand OTPM quota rejected the endpoint's default 2048-token reservation with HTTP 429; that failure also exited 1 and journaled cleanly. Exit codes 2/4 remain defined but deferred to LRN-22 (X-3); `--max-turns` is an interim CLI cap moving to LRN-19. |
...
| # | Task | Type | Size | Depends | Req | Done when | Status | Comments |
|---|---|---|---|---|---|---|---|---|
| **LRN-12** | `packages/stub-client`: the port — typed `health`, `read`, `write`, `stat`, `glob`, `grep`, `exec`, `shell`, `batch`, each taking `{maxBytes, maxMs, cwd, capability}` and returning a terminal `{status, bytes, truncated, elapsedMs}` frame. **Every method in blueprint §8.2, none extra** | Core | M | 05 | LR-FR-007 | The interface is written before any implementation exists; `write`, `shell` and `batch` may throw `NotImplemented` until M3 but are in the type | completed | port.test.ts (AC-12.1/12.2/12.3), errors.test.ts (AC-12.4 + DoD-2), dialect.test.ts (AC-12.6) — 18/18 green; pnpm run check green (lint/typecheck/build/test); tenth-method negative check fails typecheck as designed; grep node:fs/child_process over packages/stub-client/src empty (AC-12.5); boundaries suite 8/8 with new stub-client spec |
| **LRN-13** | The `local-ts` driver behind the port (in `stub-client`): real fs and spawn, **path containment** (canonicalize → verify inside root → open) and byte/time budgets. Search is **delegated to `rg --json`**, not implemented (ADR-025). **Plus the shared driver contract suite** that any driver must pass — the Rust stub reuses it verbatim in LRN-30 | Core | M | 12 | LR-FR-007, LR-FR-008, LR-FR-013 | Containment tests including `../` and a symlink out. The contract suite is a separate exported test module, not inline tests. Adversarial testing is LRN-32 | completed | tests/contract/driver.ts (24 specs, rg 15.2.0 table) run verbatim via tests/driver-contract.test.ts — 24/24 green; pnpm run check green (tests 62/62); broken-driver probe fails 1/24 as designed (deleted, uncommitted); openSync occurs once (resolve.ts), all bytes flow through the verified fd; rg --glob override fixed by intersecting with rg's own ignore-aware universe + --no-require-git |
| **LRN-13b** | **Sandbox-runtime spike.** One evening, throwaway, LRN-01-shaped: can `@anthropic-ai/sandbox-runtime` wrap a **long-lived** child process, and does it expose the **generated profile text**? Write the answers down and delete the script | Harden | S | 03 | LR-FR-010 | `docs/sandbox-runtime-notes.md` records both answers and a go/no-go. **Do this before M4 starts, not inside it** | completed | docs/sandbox-runtime-notes.md: 0.0.75 — long-lived YES (persistent child enforced, EPERM observed), profile text embedded in wrapped -p operand, no dedicated accessor — GO conditional (pin 0.0.75, test the extraction, .sb fallback named); scratch deleted, only the doc committed |
| **LRN-14** | **Boundary lint.** `dependency-cruiser` for dependency direction, plus a rule banning `child_process` and direct fs access anywhere except `stub-client`, `storage` and (later) `native/` | Harden | M | 13 | LR-FR-006 | Two planted violations each fail the lint. **Do this now, while there are three files to fix, not thirty** | completed | Biome `noRestrictedImports` (fs/fs-promises/child_process/module/os; `version.ts` refactored to `OM_VERSION` + drift test) + `lint:deps` (`protocol-is-a-leaf`, `session-only-protocol`, `kernel-ports-only`, per-adapter `*-does-not-cross`, `nothing-depends-on-cli`, `no-circular`; `tsconfig.depcruise.json` resolves workspace imports to `src` so no build is needed); AC-14.2 fixtures in `tests/fixtures/boundary-lint/` asserted via temp-tree `biome lint` diagnostics; AC-14.3 runtime + static write-scope in `tests/storage-write-scope.test.ts` (write-capable set is `{journal/files.ts, journal/writer.ts}` — `lock.ts` writes only through `files.ts` handles); cli types moved to `src/types.ts` leaf to clear 6 type-only `no-circular` hits. DoD-8: `--stdin-file-path` is unusable for AC-14.2 (stdin mode echoes source, ignores `--reporter=json`, inconsistent exit codes) — fixtures must be real files; `lock.ts` exclusion above; per-adapter generated rules because one regex cannot tell a cross-adapter edge from a self-import |
| **LRN-15** | CI: one macOS workflow — install, biome, `tsc --noEmit`, dependency-cruiser, tests | Harden | S | 14 | §11 | Green on push. One OS in the matrix | completed | `.github/workflows/ci.yml` (macos-latest + arm64 assert, brew ripgrep pinned to major 15, frozen install, biome, lint:deps, typecheck, suite with text coverage printed). Local suite wall-clock 7.6 s vs the 3-minute budget. Green: https://github.com/OmPrajapati7901/om-code/actions/runs/34158159833 (PR #2, all 12 steps incl. coverage). Red (DoD-2): planted `banned-fs` fixture as `packages/kernel/src/planted.ts` on a scratch branch failed exactly at `biome check`: https://github.com/OmPrajapati7901/om-code/actions/runs/34158317388 (PR #3, closed, branch deleted) |
| **LRN-16** | `packages/tools`: the `Tool` interface (`descriptor` / `plan` → `CapabilityRequest` / `execute`), the registry, and a test pinning the roster at 7 | Core | M | 12 | LR-FR-015, LR-FR-020 | A tool without `plan()` does not compile; an eighth tool fails a test | completed | `packages/tools` (ports/tool/roster/registry/errors/run/index; protocol+zod only): AC-16.1 via `tests/tool-type.test.ts` (temp-tree tsc, missing-plan errors naming plan, complete-tool clean); AC-16.2 via structural `ToolIo` pin `tests/tool-io-contract.test.ts` + deliberate Biome-override omission; AC-16.3/16.4 via `registry.test.ts` (duplicate → duplicate-name, eighth → not-in-roster, roster is the 7 names); AC-16.5 via `descriptor.test.ts` (parameters deep-equals fresh `z.toJSONSchema` emit + shape-keys cross-check); AC-16.6 via `run.test.ts` (plan→record→execute, plan rejection skips both) + `tests/tool-journal.test.ts` (tool_call seq < tool_result seq in a real journal). `runTool` records then executes (append-before-effect). Deviations: `src/roster.ts` leaf holds TOOL_ROSTER/ToolName (a registry home would close a type-only cycle under no-circular — same reason as cli `types.ts`); AC-5.8 left untouched per its own deferral note |
| **LRN-17** | Read tools: `read` (with line ranges), `grep`, `glob` — each with an accurate `plan()`, each going through the port | Core | M | 16 | LR-FR-020 | The model can explore a repo and provably cannot change it | completed | `readFrameSchema.totalLines` + the shared driver contract prove whole/ranged/no-final-newline/empty/truncated/timeout counting and ranged byte exactness. `packages/tools` ships `readOnlyTools()` with Zod-derived descriptors, read-only plans, port-only execution, numbered read/grep output, frame passthrough, and typed NUL/invalid-UTF-8 refusals; package tests: 24 passed. Root `read-tools.test.ts` exercises all three through `runTool` + `createLocalDriver`, including a 300-line ranged total, genuine `path-escape`, and a real NUL file; root suite: 75 passed. `pnpm run check` passed. Negative controls: a temporary tools→stub-client import failed `lint:deps` with `tools-does-not-cross`; a temporary `node:fs` import failed Biome `noRestrictedImports`; both were reverted. Manual real-repo chain: glob `packages/**/*.ts` → 134 files; grep `assemblePrompt` → 20 matches/5 files; read confirmed `packages/kernel/src/index.ts:10` with the exact numbered anchor. |
| **LRN-18** | Multi-tool turn: tool calls execute, results return to the model, the loop continues until it answers. Parallel calls run concurrently | Core | M | 10, 17 | LR-FR-019 | **`om run -p "where is auth handled?"` gives a correct answer on a real repo of yours** | completed | Code + full gate green (`pnpm run check` exit 0). Live 2026-09-07 (Groq `openai/gpt-oss-20b`, repo-local OM_HOME, transcripts in `tmp/lrn-gates/`): 4-round loop (glob + 3 refining greps, incl. a dialect-refusal failure fed back per AC-18.4), then a clean 1-glob run answering "Yes." correctly — journal seq 4 assistant(tool_calls) → 5 tool_call → 6 tool_result(ok) → 8 assistant("Yes.") → 9 turn_end, exit 0. Earlier attempts hit free-tier flakiness (OTPM 429 on qwen, TPM 413 on huge grep dumps, mid-stream errors, one empty-text completion) — endpoint-side, journaled cleanly each time. |
| **LRN-19** | **Central bounding.** The runtime — not any tool — truncates results, notes the truncation and spills the rest to a file. Turn count, wall-clock and byte caps live in this one module | Core | M | 18 | LR-FR-023, LR-NFR-003 | A 50 MB command output cannot blow the context. **Zero truncation logic inside any tool** | completed | Code + full gate green (`pnpm run check` exit 0; context 11/11 incl. 50–52 MB fuzz with zero violations, kernel 33/33, storage 63+1 skipped, cli 60/60, root 78/78; coverage printed). Live 2026-09-07 (gpt-oss-20b, repo-local OM_HOME, transcripts in `tmp/lrn-gates/`): 69,502 B read spilled — `tool_result` carries `truncated:true`, `bytes:69502`, notice + `sha256:` ref; blob file at `blobs/sha256/68/…` and `om show` renders its path (AC-19.2/19.3). `--max-turns 1` → exit 3 with journaled `max-turns` (AC-19.5). `--max-cost` startup-rejects exit 1 with no inference when pricing is absent, and with pricing enforced mid-run exit 3 (`$0.689000 spent over 1 inference(s)`, journaled `max-cost`, AC-19.6). Unknown-usage abort (exit 1) covered by automated test — no non-reporting endpoint available live. |
| **LRN-20** | Repository discovery: git root, ignore rules, and `AGENTS.md` / `CLAUDE.md` loaded into the prompt, journaled by content hash | Core | S | 09, 17 | LR-FR-024 | Works from a subdirectory of a repo. Worktrees and monorepo package detection are deferred until you need them | completed | `packages/cli/src/discovery.ts`: root probe (single-directory mode outside repos, no crash), `AGENTS.md`→`CLAUDE.md` via the stub driver with content hashes, skip-if-missing/>64 KB with warning; wired per-request into the turn. `pnpm run check` green; new `discovery.test.ts` (7), `run.test.ts` hash/order/re-read test, root gitignore proof (2), storage `findGitRoot` tests (2). Worktree/monorepo deferral recorded in code per AC-20.4. |
...
| # | Task | Type | Size | Depends | Req | Done when | Status | Comments |
|---|---|---|---|---|---|---|---|---|
| **LRN-21a** | `packages/policy`: the decision core — `CapabilityRequest` → `{outcome: allow \| ask \| deny, reason, ruleId}`, the rule representation, and precedence `deny > ask > allow` when several rules match. One tier, no modes yet | Core | M | 16 | LR-FR-016 | Tool names are not an input — asserted by the signature and a grep lint | completed | New `packages/policy` (protocol+zod only): data-form `PolicyRule` matchers (risk_class membership, `/`-boundary path prefixes, `{}` match-all), `evaluate` with deny>ask>allow precedence and fail-safe `ask` default (`ruleId: null`). `pnpm run check` green; 20 policy tests (per-pair precedence, prefix boundaries, process capabilities, schema DoD-2) + root X-4 compat test proving real `plan()` outputs evaluate unchanged; AC-21.1 grep lint green with zero scrubs. No `CapabilityRequest` reshaping needed. |
| **LRN-21b** | Tier resolution: user > project > session, merged into one decision. **A lower tier cannot loosen a higher tier's `deny`** | Core | M | 21a | LR-FR-016 | A test asserts a session rule cannot unlock what the user tier denied | completed | `resolveTiers` evaluates each tier with 21a's core and combines deny-anywhere-wins (tightening honored upward too), ask>allow, higher-tier tie-breaks, reason naming tier+rule; `parseRuleFile` validates rule-file text into a typed `PolicyError`. `pnpm run check` green; 13 tiers tests (AC-21.2b naming, AC-21.3 both directions, order-independence) + AC-21.3 through real `plan()` output at root; file I/O deferred to 21d. |
| **LRN-21c** | Modes: `read_only` (denies every `write_workspace`, `exec` and `destructive` capability) and `manual`. **Plus the full matrix test** over mode × capability class × tier — all three dimensions now exist | Core | S | 21b | LR-FR-016 | The matrix test enumerates every combination and each cell asserts an expected outcome | completed | `rulesForMode`: `manual` imposes nothing, `read_only` contributes three attributable deny rules merged into the user tier (deny-anywhere-wins keeps them global). Full 24-cell matrix green plus mode unit tests (policy suite 60/60). Design correction during implementation: tiers with no matching rule now abstain instead of contributing default-ask — otherwise silent tiers would outrank genuine allows and session grants could never work; pinned by test. |
| **LRN-21d** | **Fail-closed and journaling.** A thrown error during evaluation becomes `deny`; an unrecognized or partly parsed capability becomes `ask`; every decision is journaled with `decided_by` before the effect runs | Core | M | 21c | LR-FR-016, LR-NFR-004 | Fail-closed is proven by a test that makes evaluation throw, not by a comment saying it does | completed | `permission` v2 (ask + system attribution, v1 readable); `decide()` validates→ask / throws→deny with a visible builtin read-allow; `runTool` authorize seam (plan→record→authorize→execute, denied ends without execute, reasons stay out of tools); CLI loads tier files at startup (malformed fails exit 1, session closed cleanly) and journals permission v2 pre-effect with journal-failure abort. `pnpm run check` green; protocol 100, policy 70+, tools 26+, cli 72, root 85+ (incl. denying-tier seq-order + show line). |
