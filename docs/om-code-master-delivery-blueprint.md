...
| **LR-FR-036** | **Configuration.** `~/.om-code/config.json` plus environment overrides for endpoint base URL, model id and credential reference, with project-tier overrides. Precedence: flags > env > project `<project>/.om-code/settings.json` > user `~/.om-code/config.json` > defaults. The credential is a reference `env:<VAR>` or `keychain:<service>/<account>`, read from the environment or the macOS keychain by the host only (the host reads the project-tier file directly as application-owned configuration, not via the stub), and is never written to disk by us. `om config print --effective --with-sources` shows where each value came from | C | Every setting resolves through one loader; a test asserts tier precedence |
...
| T2 | Sandbox persistence via config files | Config-path deny list holds regardless of user rules (LR-FR-011); LRN-33 puts `~/.om-code/config.json` and `<project>/.om-code/settings.json` on it |
